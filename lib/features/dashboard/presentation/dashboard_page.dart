                  ...
                  child: SingleChildScrollView(
                    child: Column(
                      ...
                      children: [
                        ...
                        const Text('Aktif Projeler', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                        const SizedBox(height: 12),
                        SizedBox(
                          height: 140,
                          child: ListView.separated(
                            scrollDirection: Axis.horizontal,
                            itemBuilder: (_, index) => ProjectMiniCard(project: state.activeProjects[index]),
                            separatorBuilder: (_, __) => const SizedBox(width: 12),
                            itemCount: state.activeProjects.length,
                          ),
                        ),
                        const SizedBox(height: 24),
                        const Text('Yaklaşan Vadeler', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                        const SizedBox(height: 12),
                        Column(
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text('${amount / 100} ₺', style: Theme.of(context).textTheme.headlineSmall),
        ],
      ),
    );
  }
...
class ProjectMiniCard extends StatelessWidget {
  const ProjectMiniCard({super.key, required this.project});
...
class UpcomingDebtTile extends StatelessWidget {
  const UpcomingDebtTile({super.key, required this.debt});
        ...
        children: [
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('${(debt.amount / 100).toStringAsFixed(2)} ₺', style: Theme.of(context).textTheme.titleMedium),
          ...
          Chip(
            ...
            backgroundColor: remainingDays <= 2 ? Colors.redAccent : Colors.orangeAccent,
          ),
        ],
      ),
    );
